...
1. **Recon** — one tool-enabled survey of the repo via the built-in recon agent ([`src/agents/recon.md`](packages/cli/src/agents/recon.md), loaded by [`recon-agent.ts`](packages/cli/src/recon-agent.ts)). Produces a concise `ReconReport` → `state/recon.json`. Cached by `reconHash` (root + `fingerprint` tags); `--re-recon` forces a refresh. The brief is injected into precondition prompt checks and into every agent's detection prompt.
2. **Precondition** — for each selected agent, decide queued vs skipped ([`precondition.ts`](packages/cli/src/precondition.ts)). The decisions (with reasons) are written to `state/plan.json` **before any agent runs**.
...
5. **Report** — per-finding `findings/*.md` + `summary.md`.
...
└── state/
    ...
    ├── runs/<id>.json     ← RunMeta per scan / revalidate / score
...
`--rescan` bypasses resume. Changing scope (`--diff`, `--exclude`, `--only`, `--max-file-size`, root) or the recon brief (`--re-recon` / a stack change) invalidates the affected agents.
...
| `--re-recon` | recon | Re-run recon instead of reusing the cached brief. |
