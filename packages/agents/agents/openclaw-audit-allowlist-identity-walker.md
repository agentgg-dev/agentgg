---
slug: openclaw-audit-allowlist-identity-walker
name: Allowlist Identity Audit — Walker (OpenClaw chat-channel extensions)
description: Walker-mode variant of the allowlist-identity audit. Enumerates candidate files in `extensions/` (monitor, resolve, directory, refresh, pairing, account, group-policy, allowlist) and uses preFilter regexes to narrow to files that touch identity resolution or the dangerouslyAllowNameMatching flag, then runs a per-file agentic investigation with tools. Same bug class as the hunter, deterministic candidate enumeration.
version: 0.1.0
mode: walker
noiseTier: normal
filePatterns:
  ...
  - "extensions/*/src/**/monitor.*.ts"
  - "extensions/*/src/**/monitor/*.ts"
  - "extensions/*/src/**/group-policy.ts"
  - "extensions/*/src/**/pairing*.ts"
  - "extensions/*/src/**/account*.ts"
  - "extensions/*/src/**/accounts*.ts"
  - "extensions/*/src/**/resolve*.ts"
  - "extensions/*/src/**/directory*.ts"
  - "extensions/*/src/**/refresh*.ts"
  - "extensions/*/src/**/allowlist*.ts"
  - "extensions/*/src/**/handler*.ts"
  - "extensions/*/src/**/start*.ts"
  - "extensions/*/src/**/init*.ts"
  - "extensions/*/src/**/inbound*.ts"
excludePatterns:
  - "**/e2e/**"
  - "**/*test*/**"
  - "**/__tests__/**"
  - "**/fixtures/**"
preFilter:
  - regex: "mergeAllowlist"
    label: "allowlist-state-mutation"
  - regex: "summarizeMapping"
    label: "name-resolution-canary"
  - regex: "buildNameIndex"
    label: "name-index-build"
  - regex: "resolve.*Allowlist|resolve.*Entries|resolve.*Names|resolve.*Targets"
    label: "allowlist-resolution-helper"
  - regex: "isDangerousNameMatchingEnabled|dangerouslyAllowNameMatching|allowNameMatching"
    label: "name-matching-flag-touchpoint"
  - regex: "displayName|nickname|realName|userName|channelName|groupName|roomName"
    label: "mutable-identity-field"
  - regex: "list[A-Z]\\w*(Friends|Contacts|Members|Groups|Rooms|Channels|Users|Peers|Teams)|searchUsers|userDirectorySearch|getRoster|fetchDirectory|getMembers"
    label: "directory-or-roster-call"
  - regex: "account\\.config\\.(allowFrom|groupAllowFrom|groups|channels|teams)"
    label: "authorization-state-write"
maxTurnsPerBatch: 30
maxFilesPerBatch: 5
references:
  - GHSA-c29c-2q9c-pc86
