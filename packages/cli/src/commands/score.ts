...
export async function runScore(
  ...
  await runConcurrent(tasks, 1, async ({ record, finding }) => {
    ...
    try {
      ...
      console.error(
        `  score:${finding.id} failed — ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  });
...
export function registerScoreCommand(program: Command): void {
  program
    ...
    .option(
      "--model <name>",
      "One-shot model override for the selected provider (not persisted)",
    )
    .option(
      "--force",
      "re-score findings that already carry a CVSS score (default: skip them)",
    )
    .option(
    ...
    .action(async (outputDir: string, opts: ScoreOpts) => {
      try {
        ...
        console.error(
          `score failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        process.exit(1);
      }
    });
