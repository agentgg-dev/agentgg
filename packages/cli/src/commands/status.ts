...
export async function runStatus(
  ...
  if (!scanMeta) {
    if (opts.json) {
      console.log(
        JSON.stringify({ outputDir, exists: false }, null, 2),
      );
      return;
    }
  ...
  for (const f of validated) {
    ...
    verdictCounts[f.validation.verdict] =
      (verdictCounts[f.validation.verdict] ?? 0) + 1;
  }
    ...
    for (const run of runs.slice(0, 5)) {
      const duration =
        run.completedAt && run.startedAt
          ...
          : "running";
      const findingsPart =
        run.stats.findingsCount !== undefined
          ? ` findings: ${run.stats.findingsCount}`
          : "";
      const filesPart =
        run.stats.filesScanned !== undefined
          ? ` files: ${run.stats.filesScanned}`
          : "";
      console.log(
...
export function registerStatusCommand(program: Command): void {
  program
    .command("status")
    .description(
      "show scan state for an --output directory (files tracked, findings, recent runs)",
    )
    .argument(
      "[output-dir]",
