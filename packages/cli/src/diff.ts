...
function gitDiffTreeNameOnly(commit: string, cwd: string): string {
  try {
    // Range syntax (`a..b` / `a...b`) → diff between two refs (PR review
    // semantic). Bare ref → that commit's own changes vs its parent
    // (single-commit review semantic). Mirrors git's own argument
    // conventions so users don't have to learn a new vocabulary.
    if (commit.includes("..")) {
      return execFileSync("git", ["diff", "--name-only", "--diff-filter=ACMRTUB", commit], {
        cwd,
        encoding: "utf8",
      });
    }
    return execFileSync(
      "git",
      ["diff-tree", "--no-commit-id", "--name-only", "-r", "--diff-filter=ACMRTUB", commit],
      { cwd, encoding: "utf8" },
    );
  } catch (err) {
...
function gitShow(commit: string, cwd: string): string {
  ...
  if (commit.includes("..")) {
    try {
      return execFileSync("git", ["diff", commit], {
        cwd,
        encoding: "utf8",
      ...
      const suffix = stderr ? `\n  ${stderr}` : "";
      throw new Error(
        `git diff ${commit} failed.${suffix}\n` +
          `  Check that '${commit}' is a valid commit range reachable in this repo.`,
      );
    }
  }
  try {
