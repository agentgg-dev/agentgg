 ...
 *
 * Same shape as deepsec's `CandidateMatch.lineNumbers` + snippet,
 * adapted to one-hit-per-row so the prompt can render exactly which
 * line the model should anchor on.
 */
 ...
 * preFilter still gets to investigate every file (matches the
 * deepsec batch-investigate fallback for direct-invocation flows).
 */
