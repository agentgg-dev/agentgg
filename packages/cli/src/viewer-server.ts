...
export async function findFreePort(
  ...
  throw new Error(
    `Could not find a free port in [${start}, ${start + attempts - 1}]`,
  );
}
...
export async function startViewer(opts: StartOpts): Promise<ViewerHandle> {
  ...
  if (opts.port && port !== opts.port) {
    process.stdout.write(
      `Port ${opts.port} is busy; using ${port} instead.\n`,
    );
  }
  ...
  await new Promise<void>((resolveP, rejectP) => {
    ...
    const exitHandler = (code: number | null) => {
      finish(
        new Error(
          `Viewer process exited with code ${code ?? "null"} before it was ready.`,
        ),
      );
    };
