...
import { addAgents, getCustomAgentsDir, removeAgent } from "../src/agents-fs.js";
...
describe("getCustomAgentsDir", () => {
  it("resolves under AGENTGG_HOME/agents/custom", () => {
    expect(getCustomAgentsDir(env)).toBe(join(agentggHome, "agents", "custom"));
  });
});
...
describe("addAgents", () => {
  it("adds a single .md file using the slug as the destination filename", () => {
    ...
    expect(result.added[0].to).toBe(join(agentggHome, "agents", "custom", "my-detector.md"));
  ...
  it("throws when the source path doesn't exist", () => {
    expect(() => addAgents(join(scratch, "missing.md"), env)).toThrow(/No such file or directory/);
  });
...
describe("removeAgent", () => {
  it("removes a previously-added agent and returns the path", () => {
    writeFileSync(join(scratch, "a.md"), withSlug(VALID_MD, "to-remove"), "utf8");
  ...
  it("finds an agent by frontmatter slug even when the filename differs", () => {
    ...
    writeFileSync(weirdPath, withSlug(VALID_MD, "slug-in-frontmatter"), "utf8");
