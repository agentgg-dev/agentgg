...
// Synthesize a minimal file-mode agent .md so these tests don't depend on
// whatever the live agentgg-agents catalog ships. The detector is mocked, so
// the prompt body doesn't matter — only the frontmatter shape.
function writeTestAgent(dir: string, slug: string): string {
  const body = `---
slug: ${slug}
name: ${slug}
description: Synthetic file-mode agent for scan-resume tests.
mode: file
filePatterns:
  - "**/*.js"
---
