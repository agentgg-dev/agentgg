...
describe("buildUserConfig", () => {
  describe("anthropic", () => {
    ...
    it("throws when no credential is supplied (neither apiKey nor oauthToken)", () => {
      expect(() => buildUserConfig({ provider: "anthropic" })).toThrow(
    ...
    it("throws when the key is only whitespace", () => {
      expect(() =>
        buildUserConfig({ provider: "anthropic", anthropicKey: "   " }),
      ).toThrow(/no API key or OAuth token/);
    });
  ...
  describe("openai", () => {
    ...
    it("throws when no key is supplied", () => {
      expect(() => buildUserConfig({ provider: "openai" })).toThrow(
        /no API key/,
      );
    });
  });
