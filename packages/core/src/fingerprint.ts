 ...
 * downstream callers can render "skipped X because <sentinel> not present"
 * messages without re-walking the tree.
 */
...
const detectors: Detector[] = [
  ...
  (root) => {
    if (!exists(root, "host.json")) return [];
    return ["azure-functions"];
  },

  // --- Generic infra signals ---
  (root) => {
    const tags: string[] = [];
...
/**
 * Walk the project root once and collect normalized tech tags. Pure / no
 * side effects. Re-run every scan — detection is ~10ms and avoiding stale
 * cache bugs is worth more than the savings.
 */
export function fingerprint(rootPath: string): ProjectFingerprint {
...
/**
 * Known set of tags `fingerprint()` can emit. Used by `agents lint` to
 * warn when an agent declares `tech: ["djano"]` (typo) instead of
 * silently never running. Keep this in sync with the detectors above.
 */
export const KNOWN_TECH_TAGS: ReadonlySet<string> = new Set([
  "node",
  "nextjs",
  "react",
  "express",
