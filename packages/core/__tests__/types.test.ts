...
describe("UserConfig schema", () => {
  ...
  it("rejects provider:anthropic when the anthropic block is missing", () => {
    expect(() => UserConfig.parse({ provider: "anthropic" })).toThrow(/anthropic block is missing/);
  });

  it("rejects provider:openai when the openai block is missing", () => {
    expect(() => UserConfig.parse({ provider: "openai" })).toThrow(/openai block is missing/);
  });

  it("rejects provider:ollama when the ollama block is missing", () => {
    expect(() => UserConfig.parse({ provider: "ollama" })).toThrow(/ollama block is missing/);
  });
